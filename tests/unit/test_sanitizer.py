...
def test_a_result_under_the_cap_passes_through_unmodified():
    envelope = json.loads(sanitize(ToolSucceeded(name="get_physician_profile", result_json=SMALL_RESULT)))

    assert envelope == {"tool": "get_physician_profile", "result": json.loads(SMALL_RESULT)}
...
def test_every_outcome_is_enveloped_including_the_failures(outcome, expected_key):
    envelope = json.loads(sanitize(outcome))

    assert set(envelope) == {"tool", expected_key}
...
def test_the_envelope_is_built_by_one_escaping_path():
    ...
    envelope = json.loads(sanitize(UnknownTool(requested_name=hostile, valid_names=("a",))))

    assert set(envelope) == {"tool", "error"}
    assert envelope["tool"] == hostile


def test_a_vast_tool_name_cannot_take_over_the_message():
    envelope = json.loads(sanitize(UnknownTool(requested_name="n" * 10_000, valid_names=("a",))))
...
def test_an_oversized_result_is_truncated_and_says_how_much_was_omitted():
    ...
    envelope = json.loads(sanitize(ToolSucceeded(name="search_product_docs", result_json=result)))

    assert set(envelope) == {"tool", "result_partial", "omitted_chars", "note"}
...
def test_a_truncated_result_is_still_valid_json_under_a_different_key():
    ...
    envelope = json.loads(sanitize(ToolSucceeded(name="search_product_docs", result_json=big_result())))
...
def test_a_result_exactly_at_the_cap_is_not_truncated():
    ...
    assert "result" in json.loads(sanitize(ToolSucceeded(name="x", result_json=result)))
...
def test_the_serialization_is_compact_and_key_order_is_fixed():
    message = sanitize(ToolSucceeded(name="get_physician_profile", result_json=SMALL_RESULT))

    assert message.startswith('{"tool":"get_physician_profile","result":')
